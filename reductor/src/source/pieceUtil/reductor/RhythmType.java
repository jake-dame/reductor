...
public enum RhythmType {
    ...
    private static void setDurations(int resolution) {
        if (!durationsSet) {
            for (RhythmType val : RhythmType.values()) {
                val.duration = (long) (val.multiplier * resolution);
            }
            durationsSet = true;
        }
    }
    ...
    public long getDuration(int resolution) {
        setDurations(resolution);
        return this.duration;
    }

    static long getDuration(RhythmType enumVal, int resolution) {
        setDurations(resolution);
        return enumVal.duration;
    }

    public static RhythmType getEnumType(long duration, int resolution) {
